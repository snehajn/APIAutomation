...
public class TC_01_Money_Transfer_BankTransfer_Happy extends TestBase {
	...
	public void TC01MoneyTransfer_BankTransfer() throws InterruptedException, IOException {
		...
		try {
			...
			Beneficiary[] benList = M.Get_Beneficiary_BankTransfer_Pojo(UserID,FConstants.BankTransfer);
			// First Beneficiary
			...
			if (elgiresponse.getBody().asString().equalsIgnoreCase("true")) {
				...
				Response Fxresponse = M.get_Fxrates(UserID, "INR", "25", FConstants.BankTransfer);
				...
				// String
				// Transactio_ID=Transferresponse.getBody().jsonPath().get("id").toString();
				// System.out.println("Transaction was successful and the Transaction ID "
				// +Transactio_ID);
